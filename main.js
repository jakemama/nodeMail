...
//纪念日
let startDay = "2021/1/20";
// 生日
let girlBirthday = "1997/1/18"
let boyBirthday = "1997/10/3"
...
//发送者邮箱账户SMTP授权码
let EamilAuth = {
  user: "ma18510784001@163.com",
  pass: "ACEVXCGFARCFAOAR"
};
//发送者昵称与邮箱地址
...
const jakeUrl = "https://jakemama.github.io/quotes/";
const WeatherUrl = "https://tianqi.moji.com/weather/china/" + local;

// 获取Ant-Thinker上的格言警句
function getJakeData(){
  let p = new Promise(function(resolve,reject){
      superagent.get(jakeUrl).end(function(err, res) {
          if (err) {
              reject(err);
          }
...
// 判断今年纪念日有没有过
function celebrate(targetDay) {
  let today = new Date()
  let pass = false
  let targetList = targetDay.split('/')
  let monthCheck = ((today.getMonth() + 1) > (targetList[1] - 0)) ? true : false
  let dayCheck = (((today.getDate() - 0) > (targetList[2] - 0)) && (today.getMonth() + 1) == (targetList[1] - 0)) ? true : false
  if (monthCheck || dayCheck) {
    pass = true
  } else {
    pass = false
  }
  let thisYear = (pass ?  (today.getFullYear() + 1) : today.getFullYear()) +
  "/" +
  targetList[1] +
  "/" +
  targetList[2]
...
function getAllDataAndSendMail(){
    ...
    // 获取纪念日
    let target = new Date(celebrate(startDay))
    let celebrateDays = Math.floor((target - today) / 1000 / 60 / 60 / 24);
    // 获取生日
    let girlTarget = new Date(celebrate(girlBirthday))
    let boyTarget = new Date(celebrate(boyBirthday))
    let girlDay = Math.floor((girlTarget - today) / 1000 / 60 / 60 / 24);
    let boyDay = Math.floor((boyTarget - today) / 1000 / 60 / 60 / 24);
    ...
    HtmlData["celebrateDays"] = celebrateDays;
    HtmlData["girlDay"] = girlDay;
    HtmlData["boyDay"] = boyDay;

    Promise.all([getOneData(),getWeatherTips(),getWeatherData(),getJakeData()]).then(
        function(data){
            HtmlData["todayOneData"] = data[0];
            HtmlData["weatherTip"] = data[1];
            HtmlData["threeDaysData"] = data[2];
            HtmlData["jakeData"] = data[3];
            sendMail(HtmlData)
        }
    ).catch(function(err){
